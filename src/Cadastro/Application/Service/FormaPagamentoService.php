...
final class FormaPagamentoService {
    ...
    public function create(CreateFormaPagamentoRequest $r): FormaPagamento { $this->validator->validate($r); $company=$this->companyRepo->findById((int)$r->companyId); if(!$company){throw new ResourceNotFoundException('Company não encontrada.');} return $this->tx->run(function() use($r,$company): FormaPagamento { $f=new FormaPagamento((int)$company->getId(),$r->codigo,$r->nome,$r->tipo,$r->status); $this->repo->save($f); $this->audit->log((int)$company->getId(),'forma_pagamento','cadastro.forma_pagamento.created',['formaPagamentoId'=>$f->getId()]); return $f;}); }
