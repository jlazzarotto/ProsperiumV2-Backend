...
final class CentroCustoService {
    ...
    public function create(CreateCentroCustoRequest $r): CentroCusto { $this->validator->validate($r); $company=$this->companyRepo->findById((int)$r->companyId); if(!$company){throw new ResourceNotFoundException('Company não encontrada.');} $parent=null; if($r->parentId!==null){$parent=$this->repo->findById($r->parentId); if($parent===null||$parent->getCompanyId()!==(int)$company->getId()){throw new ValidationException(['parentId'=>['Centro de custo pai inválido para a company.']]);}} return $this->tx->run(function() use($r,$company,$parent): CentroCusto { $c=new CentroCusto((int)$company->getId(),$parent,$r->codigo,$r->nome,$r->status); $this->repo->save($c); $this->audit->log((int)$company->getId(),'centro_custo','cadastro.centro_custo.created',['centroCustoId'=>$c->getId()]); return $c;}); }
