...
final class CategoriaFinanceiraService {
    ...
    public function create(CreateCategoriaFinanceiraRequest $r): CategoriaFinanceira { $this->validator->validate($r); $company=$this->companyRepo->findById((int)$r->companyId); if(!$company){throw new ResourceNotFoundException('Company não encontrada.');} $parent=null; if($r->parentId!==null){$parent=$this->repo->findById($r->parentId); if($parent===null||$parent->getCompanyId()!==(int)$company->getId()){throw new ValidationException(['parentId'=>['Categoria pai inválida para a company.']]);}} return $this->tx->run(function() use($r,$company,$parent): CategoriaFinanceira { $c=new CategoriaFinanceira((int)$company->getId(),$parent,$r->codigo,$r->nome,$r->tipo,$r->status); $this->repo->save($c); $this->audit->log((int)$company->getId(),'categoria_financeira','cadastro.categoria.created',['categoriaId'=>$c->getId()]); return $c;}); }
