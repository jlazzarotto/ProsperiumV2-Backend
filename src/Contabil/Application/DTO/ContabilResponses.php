...
{
    public static function conta(ContaContabil $conta): array { return ['id' => $conta->getId(), 'companyId' => $conta->getCompanyId(), 'parentId' => $conta->getParent()?->getId(), 'codigo' => $conta->getCodigo(), 'nome' => $conta->getNome(), 'tipo' => $conta->getTipo(), 'status' => $conta->getStatus()]; }
    public static function lancamento(LancamentoContabil $lancamento, array $itens): array { return ['id' => $lancamento->getId(), 'companyId' => $lancamento->getCompanyId(), 'empresaId' => $lancamento->getEmpresa()->getId(), 'unidadeId' => $lancamento->getUnidade()->getId(), 'tituloId' => $lancamento->getTitulo()?->getId(), 'dataLancamento' => $lancamento->getDataLancamento()->format('Y-m-d'), 'historico' => $lancamento->getHistorico(), 'status' => $lancamento->getStatus(), 'itens' => array_map([self::class, 'lancamentoItem'], $itens)]; }
