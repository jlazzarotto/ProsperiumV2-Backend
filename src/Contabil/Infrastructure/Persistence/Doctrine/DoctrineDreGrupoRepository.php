...
final class DoctrineDreGrupoRepository extends ServiceEntityRepository implements DreGrupoRepositoryInterface
{
    ...
    public function listAll(int $companyId, ?string $status = null): array { $qb=$this->createQueryBuilder('g')->andWhere('g.companyId = :companyId')->setParameter('companyId',$companyId)->orderBy('g.ordem','ASC'); if($status!==null){$qb->andWhere('g.status = :status')->setParameter('status',$status);} return $qb->getQuery()->getResult(); }
}
