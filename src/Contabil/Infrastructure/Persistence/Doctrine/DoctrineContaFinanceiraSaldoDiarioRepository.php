...
final class DoctrineContaFinanceiraSaldoDiarioRepository extends ServiceEntityRepository implements ContaFinanceiraSaldoDiarioRepositoryInterface
{
    ...
    public function listByPeriodo(int $companyId, int $empresaId, int $unidadeId, \DateTimeImmutable $inicio, \DateTimeImmutable $fim): array { return $this->createQueryBuilder('s')->andWhere('s.company = :companyId')->andWhere('s.empresa = :empresaId')->andWhere('s.unidade = :unidadeId')->andWhere('s.dataSaldo BETWEEN :inicio AND :fim')->setParameter('companyId',$companyId)->setParameter('empresaId',$empresaId)->setParameter('unidadeId',$unidadeId)->setParameter('inicio',$inicio)->setParameter('fim',$fim)->orderBy('s.dataSaldo','ASC')->getQuery()->getResult(); }
}
