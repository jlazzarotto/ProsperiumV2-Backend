...
final class TituloService
{
    ...
    public function create(CreateTituloRequest $r): Titulo
    {
        ...
        return $this->tx->run(function () use ($r,$company,$empresa,$unidade,$pessoa,$conta): Titulo {
            $titulo = new Titulo((int)$company->getId(),$empresa,$unidade,$pessoa,$r->tipo,$r->numeroDocumento,number_format((float)$r->valorTotal,2,'.',''),new \DateTimeImmutable($r->dataEmissao),$r->observacoes,$conta);
    ...
    private function resolveContext(int $companyId, int $empresaId, int $unidadeId, int $pessoaId, ?int $contaId): array
    {
        ...
        if($empresa->getCompanyId()!==(int)$company->getId()||$unidade->getCompanyId()!==(int)$company->getId()||$pessoa->getCompanyId()!==(int)$company->getId()){ throw new ValidationException(['contexto'=>['Empresa, unidade e pessoa devem pertencer à mesma company.']]); }
        $conta=null; if($contaId!==null){ $conta=$this->contaRepo->findById($contaId); if($conta===null||$conta->getCompanyId()!==(int)$company->getId()||$conta->getEmpresa()->getId()!==$empresa->getId()){ throw new ValidationException(['contaFinanceiraId'=>['Conta financeira inválida para a company/empresa informada.']]); } }
