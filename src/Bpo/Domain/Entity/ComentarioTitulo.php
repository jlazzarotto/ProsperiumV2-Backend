...
class ComentarioTitulo
{
    ...
    #[ORM\ManyToOne(targetEntity: User::class)] #[ORM\JoinColumn(name: 'user_id', referencedColumnName: 'id', nullable: false, onDelete: 'CASCADE')] private User $user;
    ...
    public function __construct(Company $company, Empresa $empresa, UnidadeNegocio $unidade, Titulo $titulo, User $user, string $comentario) { $this->company = $company; $this->empresa = $empresa; $this->unidade = $unidade; $this->titulo = $titulo; $this->user = $user; $this->comentario = trim($comentario); $this->createdAt = new \DateTimeImmutable(); }
    public function getId(): ?int { return $this->id; } public function getTitulo(): Titulo { return $this->titulo; } public function getUser(): User { return $this->user; } public function getComentario(): string { return $this->comentario; } public function getCreatedAt(): \DateTimeImmutable { return $this->createdAt; }
}
