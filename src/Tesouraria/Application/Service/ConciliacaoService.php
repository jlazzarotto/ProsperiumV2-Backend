...
final class ConciliacaoService
{
    ...
    public function create(CreateConciliacaoRequest $r): ConciliacaoBancaria
    {
        ...
        if($extrato->getCompany()->getId()!==$r->companyId){ throw new ValidationException(['companyId'=>['Extrato bancário não pertence à company informada.']]); }
        $movimento=null; $baixa=null;
        if($r->movimentoFinanceiroId!==null){ $movimento=$this->movimentoRepo->findById($r->movimentoFinanceiroId); if($movimento===null||$movimento->getCompany()->getId()!==$extrato->getCompany()->getId()){ throw new ValidationException(['movimentoFinanceiroId'=>['Movimento financeiro inválido para o extrato.']]); } }
        if($r->baixaId!==null){ $baixa=$this->baixaRepo->findById($r->baixaId); if($baixa===null||$baixa->getCompany()->getId()!==$extrato->getCompany()->getId()){ throw new ValidationException(['baixaId'=>['Baixa inválida para o extrato.']]); } }
        return $this->tx->run(function() use($extrato,$movimento,$baixa,$r): ConciliacaoBancaria { $extrato->conciliar($movimento,$baixa); $this->extratoRepo->save($extrato); $conciliacao=new ConciliacaoBancaria($extrato->getCompany(),$extrato->getEmpresa(),$extrato->getUnidade(),$extrato,$movimento,$baixa,$r->modo); $this->repo->save($conciliacao); $this->audit->log((int)$extrato->getCompany()->getId(),'conciliacao_bancaria','tesouraria.conciliacao.criada',['conciliacaoId'=>$conciliacao->getId(),'extratoId'=>$extrato->getId()]); return $conciliacao; });
    }
