...
final class ExtratoBancarioService
{
    ...
    public function importar(ImportarExtratoRequest $r): array
    {
        ...
        if($empresa->getCompanyId()!==$company->getId()||$unidade->getCompanyId()!==$company->getId()||$conta->getCompanyId()!==$company->getId()||$conta->getEmpresa()->getId()!==$empresa->getId()){ throw new ValidationException(['contexto'=>['Company, empresa, unidade e conta financeira devem ser coerentes.']]); }
    ...
    public function sugerir(ExtratoBancario $extrato): array { $regras=$this->regrasRepo->listActiveByCompany((int)$extrato->getCompanyId()); $movimentos=$this->movimentoRepo->listAll((int)$extrato->getCompanyId(),(int)$extrato->getEmpresa()->getId(),(int)$extrato->getUnidade()->getId(),null,(int)$extrato->getContaFinanceira()->getId()); return $this->sugestaoService->sugerir($extrato,$regras,$movimentos); }
}
