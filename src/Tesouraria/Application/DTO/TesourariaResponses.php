...
{
    public static function extrato(ExtratoBancario $e, ?array $sugestao = null): array { return ['id'=>$e->getId(),'companyId'=>$e->getCompanyId(),'empresaId'=>$e->getEmpresa()->getId(),'unidadeId'=>$e->getUnidade()->getId(),'contaFinanceiraId'=>$e->getContaFinanceira()->getId(),'codigoExterno'=>$e->getCodigoExterno(),'dataMovimento'=>$e->getDataMovimento()->format('Y-m-d'),'valor'=>$e->getValor(),'tipo'=>$e->getTipo(),'descricao'=>$e->getDescricao(),'status'=>$e->getStatus(),'movimentoFinanceiroId'=>$e->getMovimentoFinanceiro()?->getId(),'baixaId'=>$e->getBaixa()?->getId(),'sugestao'=>$sugestao]; }
